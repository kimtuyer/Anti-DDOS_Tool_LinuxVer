...
XDP-Per-CPU-Map 최적화 적용 이전 테스트 (멀티코어 공유 map 접근)
https://youtu.be/TSoUMaGN6Ck
XDP-Per-CPU-Map 최적화 적용 테스트
https://youtu.be/nxd7ltyqchE
...
---2026.1.23일자 업데이트---
[추가 섹션: 하이브리드 탐지 구조 도입에 따른 성능 임팩트 분석] 
배경: 단순 임계치 차단을 넘어, 정상 사용자를 오차단 없이 보호하기 위한 '화이트리스트 
박탈/승격' 기능을 유저모드 분석 엔진에 결합->커널 XDP맵 정책 적용.

현상: Pure XDP 기반 차단 시 1% 미만이었던 CPU 점유율이 하이브리드 모드(XDP + 
NFQUEUE) 가동 및 Rand-IP 공격 시 약 19%로 상승
